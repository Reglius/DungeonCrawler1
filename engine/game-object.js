...
export default class GameObject {
    ...
    draw(layers) {//How does the game object draw itself?
        ...
        if (this.name === "YellowKey" && Keys.key1){
            ctx.rotate(Math.sin(Time.tick/10));
        }
        else if (this.name === "RedKey" && Keys.key2){
            ctx.rotate(Math.sin(Time.tick/10));
        }
        ...
        for (let child of this.transform.children) {
            if (child.name === "YellowKey" && Keys.key1){
                child.draw(layers);
            }
            else if (child.name === "RedKey" && Keys.key2){
                child.draw(layers);
            }
            else if (child.name !== "YellowKey" && child.name !== "RedKey") {
                child.draw(layers);
            }
        }
        for (let layer of layers) {
